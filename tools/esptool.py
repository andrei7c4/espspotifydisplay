...
def elf2image(args):
    ...
    font_offs = e.get_symbol_addr("_font0_text_start") - 0x40200000
    ...
    if args.version == '1':
        ...
        fontdata = e.load_section(".font0.text")
        if irom_offs < 0:
            raise FatalError('Address of symbol _irom0_text_start in ELF is located before flash mapping address. Bad linker script?')        
        if font_offs < 0:
            raise FatalError('Address of symbol _font0_text_start in ELF is located before flash mapping address. Bad linker script?')
        ...
        if (font_offs & 0xFFF) != 0:  # font0 isn't flash sector aligned
            print "WARNING: font0 section offset is 0x%08x. ELF is probably linked for 'elf2image --version=2'" % font_offs
        with open(args.output + "0x%05x.bin" % irom_offs, "wb") as f:
            ...
            print "------------------------------------------------------------------------------"
            sec_name = ["data", "rodata", "bss", "lit4", "text", "irom0_text", "font0_text"]
            sec_des = ["Initialized Data (RAM)", "ReadOnly Data (RAM)", "Uninitialized Data (RAM)", "Uninitialized Data (IRAM)", "Uncached Code (IRAM)", "Cached Code (SPI)", "Font data (SPI)"]
            sec_size = []
