...
LOCAL int ICACHE_FLASH_ATTR uart0_rx_one_char() {
 ...
 * Returns      : NONE
*******************************************************************************/
extern void onUartCmdReceived(char* command, int length);
LOCAL void uart0_rx_intr_handler(void *para)
{
    ...
    while (READ_PERI_REG(UART_STATUS(UART0)) & (UART_RXFIFO_CNT << UART_RXFIFO_CNT_S)) {
		...
		if (RcvChar == '\r') {
			...
			if (str) {
				int i;
				for (i = 0; i < len; i++) str[i] = uart0_rx_one_char();
				str[len] = '\0';
				onUartCmdReceived(str, len);
				os_free(str);
			}
